...
cron.schedule("*/15 14-23,0-3 * 3 *", async () => {
  ...
  const timeOfDay =
    currentHour >= 0 && currentHour < 4 ? "late night" : "afternoon/evening";
  console.log(
    `[${new Date().toISOString()}] Running scheduled tournament update (${timeOfDay})...`
  );
  await runUpdate();
});
...
cron.schedule("*/30 8-13 * 3 *", async () => {
  console.log(
    `[${new Date().toISOString()}] Running morning tournament update...`
  );
  await runUpdate();
});
...
async function runUpdate() {
  try {
    ...
    if (settings && !settings.enabled) {
      console.log(
        `[${new Date().toISOString()}] Scheduler is disabled. Skipping update.`
      );
      return;
    }
    ...
    if (
      ...
      new Date(settings.nextRunTime) > new Date()
    ) {
      console.log(
        `[${new Date().toISOString()}] Next run time is ${
          settings.nextRunTime
        }. Skipping update.`
      );
      return;
    ...
    if (isLateNight) {
      console.log(
        `[${new Date().toISOString()}] Late night update - checking yesterday's completion status...`
      );
    ...
    if (allComplete) {
      console.log(
        `[${new Date().toISOString()}] All games for ${
          isLateNight ? "yesterday" : "today"
    ...
    if (recentLog) {
      console.log(
        `[${new Date().toISOString()}] Recent update found from ${
          recentLog.runDate
        }. Skipping to avoid redundant updates.`
      );
      return;
    }
    ...
    console.log(
      `[${new Date().toISOString()}] Update completed with status: ${
        result.status
      }`
    );

    // Check if we should disable scheduler until tomorrow
    // Check if we should disable scheduler until tomorrow or just until 8 AM today
    if (
      result.status === "success" &&
      result.totalGames > 0 &&
      result.allComplete === true
    ) {
      // Games completed during normal hours - disable until tomorrow
      await autoDisableUntilTomorrow("All games for today are complete");
      console.log(
        `[${new Date().toISOString()}] Scheduler auto-disabled until next morning - all games complete`
      );
    } else if (
      result.status === "no_updates" &&
      result.message === "No tournament games found today"
    ) {
      // No games today - disable until tomorrow
      await autoDisableUntilTomorrow("No tournament games scheduled for today");
      console.log(
        `[${new Date().toISOString()}] Scheduler auto-disabled until next morning - no games found`
      );
    } else if (result.status === "complete_for_day") {
      // Games completed during normal hours and already marked complete
      await autoDisableUntilTomorrow("Day already marked as complete");
      console.log(
        `[${new Date().toISOString()}] Scheduler auto-disabled until next morning - day marked complete`
      );
    } else if (result.status === "complete_for_yesterday") {
      // Games completed after midnight - only disable until 8 AM today
      await autoDisableUntilLaterToday("Yesterday's games are complete");
      console.log(
        `[${new Date().toISOString()}] Scheduler auto-disabled until 8 AM today - yesterday's games complete`
      );
    } else {
      console.log(
        `[${new Date().toISOString()}] Keeping scheduler enabled - status: ${
          result.status
        }`
      );
    }
    ...
    try {
      const errorLog = new NcaaUpdateLog({
        status: "error",
        logs: [
          `[${new Date().toISOString()}] Critical error in scheduler: ${
            error.message
          }`,
        ],
        errorDetails: [
...
/**
 * Auto-disable scheduler until 8 AM today
 */
async function autoDisableUntilLaterToday(reason) {
  try {
    const SchedulerSettings = require("./models/SchedulerSettings");
    ...
    // Calculate today at 8 AM
    const today8am = new Date();
    today8am.setHours(8, 0, 0, 0);

    // If it's already past 8 AM, don't disable
    if (new Date() >= today8am) {
      console.log(
        `[${new Date().toISOString()}] Not disabling scheduler - it's already past 8 AM`
      );
      return;
    }
    ...
    settings.nextRunTime = today8am;
    settings.lastUpdated = new Date();

    await settings.save();

    console.log(
      `[${new Date().toISOString()}] Scheduler auto-disabled until ${today8am.toISOString()} today. Reason: ${reason}`
    );
  } catch (error) {
...
async function checkAndMarkYesterdayCompletion() {
  try {
    ...
    if (existingCompletionLog) {
      console.log(
        `[${new Date().toISOString()}] Yesterday is already marked as complete.`
      );
      return;
    }
    ...
    const allComplete = mostRecentLog.trackedGames.every(
      (game) => game.completed
    );

    if (allComplete && !mostRecentLog.allGamesComplete) {
      console.log(
        `[${new Date().toISOString()}] All games for yesterday appear complete but weren't marked. Updating status.`
      );
...
function setupRoutes(app, auth, admin) {
  ...
  app.post(
    "/api/admin/mark-yesterday-complete",
    [auth, admin],
    async (req, res) => {
      try {
  ...
  app.get("/api/admin/tournament-today", [auth, admin], async (req, res) => {
    try {
      ...
      const latestWithGames = logs.find(
        (log) => log.trackedGames && log.trackedGames.length > 0
      );
...
async function autoDisableUntilTomorrow(reason) {
  try {
    ...
    console.log(
      `[${new Date().toISOString()}] Scheduler auto-disabled until ${nextRunTime.toISOString()}. Reason: ${reason}`
    );
  } catch (error) {
    console.error("Error auto-disabling scheduler:", error);
  }
