...
const NCAA_API_BASE = 'https://data.ncaa.com/casablanca/scoreboard/basketball-men/d1';
...
async function updateTournamentResults() {
    ...
    try {
      ...
      if (currentHour >= 0 && currentHour < 4) {
        ...
        dateToUse = yesterday;
        updateLog.addLog(`Current time is ${currentHour}:${now.getMinutes()}, using yesterday's date for API call`);
      } else {
      ...
      // Check if we already have a log entry saying all games are complete for today
      // Note: We're checking using the actual calendar day, not the adjusted dateToUse
      ...
      if (existingCompleteLog) {
        ...
        updateLog.addLog('All games for today are already complete. Skipping update.');
        await updateLog.save();
        console.log('All games for today are already complete. Skipping update.');
        return { status: 'complete_for_day', message: 'All games already complete for today' };
      }
      ...
      const tournamentGames = todayGames.filter(game => 
        game.bracketRound && game.bracketRound !== ''
      );
      ...
      if (tournamentGames.length === 0) {
        ...
        return { status: 'no_updates', message: 'No tournament games found today' };
      }
    
    // 4. Track the tournament games for today
    updateLog.addLog(`Found ${tournamentGames.length} tournament games to track`);
    ...
    updateLog.completedGames = updateLog.trackedGames.filter(game => game.completed).length;
    
    // Check if any games are complete
    const completedGames = tournamentGames.filter(game => game.gameState === 'final');
    ...
    if (completedGames.length === 0) {
      ...
      return { status: 'no_updates', message: 'No completed tournament games found' };
    }
    ...
    for (const game of completedGames) {
      ...
      const existingGame = tournament.games.find(g => g.matchupId === matchupId);
      
      if (existingGame && existingGame.completed) {
        updateLog.addLog(`Game ${matchupId} (${game.away.names.short} vs ${game.home.names.short}) already processed`);
    ...
    if (updatedCount > 0) {
      ...
      tournament.completedRounds = [...newlyCompletedRounds].sort((a, b) => a - b);
      ...
      updateLog.addLog(`Tournament update successful! Updated ${updatedCount} games.`);
      updateLog.addLog(`Completed rounds: ${tournament.completedRounds.join(', ')}`);
    } else {
      updateLog.status = 'no_updates';
...
function mapNcaaGameToMatchupId(ncaaGame, tournament) {
  ...
  for (const matchup of tournament.results[round]) {
    ...
    const teamsMatch = (
      (matchTeamNames(matchup.teamA?.name, awayTeamName) && matchTeamNames(matchup.teamB?.name, homeTeamName)) ||
      (matchTeamNames(matchup.teamA?.name, homeTeamName) && matchTeamNames(matchup.teamB?.name, awayTeamName))
    );
...
function matchTeamNames(ourName, ncaaName) {
  ...
  for (const [alias, variants] of Object.entries(aliases)) {
    if ((our.includes(alias) && variants.some(v => ncaa.includes(v))) || 
...
function manualMatchupMapping(ncaaGame, tournament, round) {
  ...
  console.log(`Manual mapping needed for game: ${teamPair} (Round ${round}, Region ${ncaaGame.bracketRegion})`);
  return null;
}
...
async function updateGameInDatabase(ncaaGame, matchupId, tournament) {
  try {
    ...
    if (gameIndex === -1) {
      console.log(`Game with matchup ID ${matchupId} not found in our database`);
      return false;
    }
    ...
    if (tournament.games[gameIndex].completed) {
      ...
      const ncaaWinner = ncaaGame.away.winner ? ncaaGame.away.names.short : ncaaGame.home.names.short;
    ...
    for (const round in tournament.results) {
      const matchupIndex = tournament.results[round].findIndex(m => m.id === matchupId);
    ...
    const loser = winner.name === currentGame.teamA.name ? { ...currentGame.teamB } : { ...currentGame.teamA };
    
    if (tournament.teams[loser.name]) {
      tournament.teams[loser.name] = {
        ...tournament.teams[loser.name],
        eliminated: true,
        eliminationRound: parseInt(matchupId < 32 ? 1 : matchupId < 48 ? 2 : matchupId < 56 ? 3 : matchupId < 60 ? 4 : matchupId < 62 ? 5 : 6),
    ...
    console.log(`Successfully updated game ${matchupId}: ${currentGame.teamA.name} ${scoreA}-${scoreB} ${currentGame.teamB.name}. Winner: ${winner.name}`);
    return true;
  } catch (error) {
    ...
    for (const bracket of brackets) {
      ...
      for (let round = 1; round <= 6; round++) {
        ...
        for (const tournamentMatchup of tournamentMatchups) {
          ...
          const bracketMatchup = bracketMatchups.find(m => m.id === tournamentMatchup.id);
